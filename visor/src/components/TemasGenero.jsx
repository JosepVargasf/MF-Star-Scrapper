...
const FEMALE_COLOR = '#A26579'
const MALE_COLOR   = '#5B6670'
...
function GenderChart({ data, title, accent, fontSize }) {
  return (
    <div className="tg-chart-wrap">
      <div className="tg-chart-title" style={{ borderColor: accent }}>{title}</div>
      {data.length === 0
        ...
        : (
          <ResponsiveContainer width="100%" height={Math.max(280, data.length * 36)}>
            <BarChart data={data} layout="vertical" margin={{ left: 8, right: 48, top: 4, bottom: 4 }}>
              ...
              <XAxis type="number" domain={[0, 100]} unit="%" tick={{ fontSize, fill: '#94a3b8' }} axisLine={false} tickLine={false} />
              <YAxis type="category" dataKey="tema" width={150} tick={{ fontSize, fill: '#475569' }} axisLine={false} tickLine={false} />
              ...
              <Bar dataKey="otros"  name="Sin clasif." stackId="a" fill={OTROS_COLOR}  maxBarSize={22} radius={[0,4,4,0]}>
                <LabelList dataKey="total" position="right" style={{ fontSize, fill: '#64748b', fontWeight: 600 }} />
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        )
    ...
    <div className="card card-full">
      <div ref={chartRef}>
        ...
        <div className="tg-cols">
          <GenderChart data={positivos} title="👍 Temas en reseñas positivas" accent="#96323C" fontSize={fontSize} />
